...
// The is identical to the PerPixelMaterial in GopherGfx.
// All code for this lecture is in the shaders.
// You do not need to go through the TypeScript code.

// @ts-ignore
import phongVertexShader from './shaders/perpixel.vert'
// @ts-ignore
import phongFragmentShader from './shaders/perpixel.frag'

import * as gfx from 'gophergfx'

export class PerPixelMaterial extends gfx.Material3
{
    ...
    constructor()
    {
        ...
        this.shininessUniform = PerPixelMaterial.shader.getUniform(this.gl, 'shininess');

        this.textureUniform = PerPixelMaterial.shader.getUniform(this.gl, 'textureImage');
        this.useTextureUniform = PerPixelMaterial.shader.getUniform(this.gl, 'useTexture');

        this.eyePositionUniform = PerPixelMaterial.shader.getUniform(this.gl, 'eyePosition');
        this.viewUniform = PerPixelMaterial.shader.getUniform(this.gl, 'viewMatrix');
        this.modelUniform = PerPixelMaterial.shader.getUniform(this.gl, 'modelMatrix');
        this.projectionUniform = PerPixelMaterial.shader.getUniform(this.gl, 'projectionMatrix');
        this.normalUniform = PerPixelMaterial.shader.getUniform(this.gl, 'normalMatrix');

        this.numLightsUniform = PerPixelMaterial.shader.getUniform(this.gl, 'numLights');
        this.lightTypesUniform = PerPixelMaterial.shader.getUniform(this.gl, 'lightTypes');
        this.lightPositionsUniform = PerPixelMaterial.shader.getUniform(this.gl, 'lightPositions');
        this.ambientIntensitiesUniform = PerPixelMaterial.shader.getUniform(this.gl, 'ambientIntensities');
        this.diffuseIntensitiesUniform = PerPixelMaterial.shader.getUniform(this.gl, 'diffuseIntensities');
        this.specularIntensitiesUniform = PerPixelMaterial.shader.getUniform(this.gl, 'specularIntensities');

        this.positionAttribute = PerPixelMaterial.shader.getAttribute(this.gl, 'position');
